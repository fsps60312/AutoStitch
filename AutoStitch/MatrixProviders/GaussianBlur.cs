    ...
    sealed class GaussianBlur : GaussianInternal.GaussianBlurBase
    {
        ...
        protected override MyMatrix GetMatrixInternal()
        {
            ...
            else
            {
                ...
                double[,] ws = new double[height, width];
                double eps = 1e-9;
                for (int dx = 0; dx < width && Gaussion(dx, 0) >= eps; dx++)
                {
                    for (int dy = 0; dy < height && Gaussion(dx, dy) >= eps; dy++)
                    {
                        double w = Gaussion(dx, dy);
                        for (int i = 0; i < height; i++)
                        {
                            for (int j = 0; j < width; j++)
                            {
                                if (0 <= j + dx && j + dx < width && 0 <= i + dy && i + dy < height) { sum[i, j] += w * image.data[i + dy, j + dx]; ws[i, j] += w; }
                                if (dx != 0)
                                {
                                    dx = -dx;
                                    if (0 <= j + dx && j + dx < width && 0 <= i + dy && i + dy < height) { sum[i, j] += w * image.data[i + dy, j + dx]; ws[i, j] += w; }
                ...
                for (int i = 0; i < height; i++) for (int j = 0; j < width; j++) sum[i, j] /= ws[i, j];
    ...
    sealed class GaussianBlurX : GaussianInternal.GaussianBlurBase
    {
        ...
        protected override MyMatrix GetMatrixInternal()
        {
            ...
            else
            {
                ...
                for (int dx = 0; dx < width && Gaussion(dx, 0) >= eps; dx++)
                {
                    double w = Gaussion(dx, 0);
                    for (int i = 0; i < height; i++)
                    {
                        for (int j = 0; j < width; j++)
                        {
                            if (0 <= j + dx && j + dx < width) { sum[i, j] += w * image.data[i, j+dx]; ws[i, j] += w; }
                            if (dx != 0)
                            {
                                dx = -dx;
                                if (0 <= j + dx && j + dx < width) { sum[i, j] += w * image.data[i, j + dx]; ws[i, j] += w; }
    ...
    sealed class GaussianBlurY : GaussianInternal.GaussianBlurBase
    {
        ...
        protected override MyMatrix GetMatrixInternal()
        {
            ...
            else
            {
                ...
                for (int dy = 0; dy < height && Gaussion(0, dy) >= eps; dy++)
                {
                    double w = Gaussion(0, dy);
                    for (int i = 0; i < height; i++)
                    {
                        for (int j = 0; j < width; j++)
                        {
                            if (0 <= i + dy && i + dy < height) { sum[i, j] += w * image.data[i + dy, j]; ws[i, j] += w; }
                            if (dy != 0)
                            {
                                dy = -dy;
                                if (0 <= i + dy && i + dy < height) { sum[i, j] += w * image.data[i + dy, j]; ws[i, j] += w; }
